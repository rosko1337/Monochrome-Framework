...
void Graphics::GetMonitorDpi(float* dpiX, float* dpiY)
{
...
bool Graphics::Init(HWND hWnd)
{
	...
	HRESULT result = Factory->CreateHwndRenderTarget(D2D1::RenderTargetProperties(), 
		D2D1::HwndRenderTargetProperties(hWnd, D2D1::SizeU(rect.right, rect.bottom), D2D1_PRESENT_OPTIONS_IMMEDIATELY), &RenderTarget);

	if (result != S_OK) return false;

	result = DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), (IUnknown**)&WriteFactory);
	if (result != S_OK) return false;
...
void Graphics::drawText(const std::string text, std::string font, int fontSize, DWRITE_FONT_STYLE fontStyle, DWRITE_FONT_WEIGHT fontWeight,
...
void  Graphics::drawText(const std::wstring text, std::string font, int fontSize, DWRITE_FONT_STYLE fontStyle, DWRITE_FONT_WEIGHT fontWeight,
	float xPos, float yPos, float width, float height, uint8_t r, uint8_t g, uint8_t b, uint8_t a,
	DWRITE_TEXT_ALIGNMENT textAllignment, DWRITE_PARAGRAPH_ALIGNMENT paragraphAllignment)
{
	float red = ConvertUI8ToFloat(r);
...
void __stdcall Graphics::GetWidthOfText(std::string text, std::string font, int fontSize, DWRITE_FONT_STYLE fontStyle, DWRITE_FONT_WEIGHT fontWeight, DWRITE_TEXT_ALIGNMENT textAllignment, float maxWidth, float maxHeight,
	float* textWidth)
{
	...
	h = WriteFactory->CreateTextLayout(std::wstring(text.begin(), text.end()).c_str(), text.size(), format, maxWidth, maxHeight, &layout);
	if (h != S_OK) { return; } // failed
	...
	*textWidth = textMetrics.widthIncludingTrailingWhitespace; // width of all text in pixels
